...
在接下来的组合式 API 学习中实例代码可能和官方文档稍有不同,我使用的是[单文件组件](https://v3.cn.vuejs.org/api/sfc-script-setup.html#%E5%9F%BA%E6%9C%AC%E8%AF%AD%E6%B3%95)`<script setup>`
...
响应式状态需要明确使用响应式 APIs 来创建。和从 `setup()` 函数中返回值一样，ref 值在模板中使用的时候会自动解包(不需要`.value`)：
...
> 即`setup`内部的变量不能是`props`或者`emits`
